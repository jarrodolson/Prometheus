...
##Note: Seasons$madnessStart manually coded from Wikipedia based on Kaggle "seasons.csv"
seasons$madnessStart <- as.POSIXct(strptime(seasons$madnessStart,format="%m/%d/%Y"))
...
##Need to get game dates
teams$year <- sapply(strsplit(teams$GameName,"/"),"[",3)
teams$month <- sapply(strsplit(teams$GameName,"/"),"[",4)
teams$day <- sapply(strsplit(teams$GameName,"/"),"[",5)
teams$gameDate <- as.POSIXct(paste(teams$year,teams$month,teams$day,sep="-"))

##Code march madness games
teams <- merge(teams,seasons,by.x="Season",by.y="years",all.x=TRUE)
teams$madness <- 0
teams$madness[teams$gameDate>=teams$madnessStart] <- 1
